...
class Table {
        ...
        if (this.thead) {
            ...
            if (this.checkAllCheckBox) {
                this.checkAllCheckBox.addEventListener("click", (e) => {
                    const checkboxes = this.rows.map((row) => row.querySelector('input[type="checkbox"]')).filter((input) => input && !input.parentElement.classList.contains("form-switch"))
    ...
    setupCheckboxListeners() {
        const checkboxes = this.table.querySelectorAll('input[type="checkbox"]:not(.form-switch > input)')
    ...
    alert(message) {
        ...
        if (!modalElement) {
            console.error("CustomTable: Elements not found.")
            return { modal: null, confirmButton: null }
        }
...
class CustomTable {
    ...
    init() {
        ...
        if (tableElements.length === 0) {
            console.error(`CustomTable: No elements found for selector ${this.tableSelector}`)
            return
        }
