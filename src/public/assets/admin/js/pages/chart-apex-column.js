...
new CustomApexChart({
    ...
    options: () => ({
        ...
        colors: [theme("primary-rgb", 0.7), theme("chart-primary"), theme("secondary-rgb", 0.7), theme("chart-secondary")],
        yaxis: {
            labels: {
                formatter: (val) => {
                    return val / 1000 + "K"
...
const getYearlyOptions = () => {
    ...
    const quarterChartEl = getElement("#chart-quarter")
    const yearChartEl = getElement("#chart-year")
...
if (yearChart && quarterChart) {
    ...
    const observer = new MutationObserver(() => {
        ...
        if (!yearChartEl || !quarterChartEl) {
            console.error("Elements chart-year or chart-quarter not found")
        }

        chart.updateOptions(getYearlyOptions())

        // Cleanup previous interval and timeout if they exist
        const intervalId = chart.w.globals.intervalId
        const timeoutId = chart.w.globals.timeoutId
        if (intervalId) {
        ...
        chart.w.globals.intervalId = setTimeout(() => {
            ...
            // Cleanup the interval and timeout once they are not needed
            clearInterval(chart.w.globals.intervalId)
            delete chart.w.globals.intervalId
        }, 100)
    })
