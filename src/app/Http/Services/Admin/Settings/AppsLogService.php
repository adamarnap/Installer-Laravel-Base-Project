    ...
    public function getLogFilesForDataTable()
    {
        ...
        return DataTables::of($files)
            ...
            ->addColumn('aksi', function ($row) {
                ...
                $user = auth()->user();
                $canRead = $user && ($user->hasRole(\App\Enums\Settings\RoleEnum::DEVELOPER->value) || $user->can('settings-apps-log.read'));
                $canDelete = $user && ($user->hasRole(\App\Enums\Settings\RoleEnum::DEVELOPER->value) || $user->can('settings-apps-log.delete'));

                if ($canRead) {
                    $btnView = '<a href="' . route('settings.apps-log.show', $row['name']) . '" title="Lihat isi log"
                        class="w-[30px] h-[30px] flex items-center justify-center border rounded-[5px] text-primary hover:bg-light-900 hover:text-primary cursor-pointer">
    ...
    public function getLogEntriesForDataTable(string $filename, array $params = [])
    {
        ...
        return DataTables::of($detail['entries'])
            ...
            ->addColumn('aksi', function ($row) {
                ...
                return '<div class="action-icon inline-flex gap-2 items-center">
                    <button type="button" title="Lihat Detail Log" class="btn-log-detail w-[30px] h-[30px] flex items-center justify-center border rounded-[5px] text-primary hover:bg-light-900 hover:text-primary cursor-pointer" data-log="' . $jsonLog . '">
                        <i class="ti ti-eye text-[16px]"></i>
                    </button>
                </div>';
            })
