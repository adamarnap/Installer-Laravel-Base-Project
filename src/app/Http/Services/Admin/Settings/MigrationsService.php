    ...
    public function getMigrationsForDataTable()
    {
        ...
        return DataTables::of($migrations)
            ...
            ->addColumn('migration_name', function ($row) {
                $fileIcon = $row['file_exists'] ? '<i class="iconify tabler--code text-primary text-base"></i>' : '<i class="iconify tabler--database text-warning text-base"></i>';
            ...
            ->addColumn('status_badge', function ($row) {
                if ($row['status'] === 'RAN') {
                    return '<span class="inline-flex items-center py-1 px-2.5 rounded-full text-xs font-semibold bg-success/10 text-success border border-success/20"><i class="iconify tabler--check text-sm mr-1"></i> Sudah Dijalankan</span>';
                }
                if ($row['status'] === 'RAN_DB_ONLY') {
                    return '<span class="inline-flex items-center py-1 px-2.5 rounded-full text-xs font-semibold bg-warning/10 text-warning border border-warning/20"><i class="iconify tabler--database text-sm mr-1"></i> DB Only</span>';
                }
                return '<span class="inline-flex items-center py-1 px-2.5 rounded-full text-xs font-semibold bg-danger/10 text-danger border border-danger/20 animate-pulse"><i class="iconify tabler--alert-triangle text-sm mr-1"></i> Belum Dijalankan</span>';
            })
            ->addColumn('aksi', function ($row) {
                ...
                if (!$row['is_ran'] && $row['file_exists']) {
                    return '<button type="button" class="btn-run-single-migration btn bg-primary border border-primary text-white hover:bg-primary-hover py-1 px-2.5 rounded text-xs inline-flex items-center gap-1.5 shadow-sm"
                        data-migration="' . e($row['name']) . '"
                        data-file="' . e($row['file']) . '"
                        data-command="php artisan migrate --path=database/migrations/' . e($row['file']) . '">
                        <i class="iconify tabler--player-play text-sm"></i> Jalankan Migrasi Ini
                    </button>';
                }

                return '<span class="text-xs text-gray-400 inline-flex items-center gap-1"><i class="iconify tabler--check text-success text-sm"></i> Selesai</span>';
            })
