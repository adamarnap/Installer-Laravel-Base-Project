...
class QueuesService
{
    ...
    public function getPendingJobsForDataTable()
    {
        ...
        return DataTables::of($jobs)
            ...
            ->addColumn('queue_badge', function ($row) {
                return '<span class="px-[8px] py-[3px] inline-block bg-primary-50 dark:bg-[#15203c] text-primary-500 rounded-sm font-medium text-xs">' . e($row->queue) . '</span>';
            })
    ...
    public function getFailedJobsForDataTable()
    {
        ...
        return DataTables::of($failedJobs)
            ...
            ->addColumn('queue_badge', function ($row) {
                return '<span class="px-[8px] py-[3px] inline-block bg-danger-100 dark:bg-[#15203c] text-danger-600 rounded-sm font-medium text-xs">' . e($row->queue) . '</span>';
            })
            ...
            ->addColumn('aksi', function ($row) {
                ...
                if (auth()->user()->can('settings-queues.update')) {
                    $btnRetry = '<form action="' . route('settings.queues.retry', $row->id) . '" method="POST" class="inline">
                        ' . csrf_field() . '
                        <button type="submit" class="text-primary-500 leading-none custom-tooltip" id="customTooltip" data-text="Retry Job">
                            <i class="material-symbols-outlined !text-md">refresh</i>
                        </button>
                    </form>';
                }

                if (auth()->user()->can('settings-queues.delete')) {
                    $btnForget = '<button type="button"
                        data-id="' . $row->id . '"
                        data-url-action="' . route('settings.queues.forget', $row->id) . '"
                        class="btn-forget-job text-danger-500 leading-none custom-tooltip" id="customTooltip" data-text="Hapus Failed Job">
