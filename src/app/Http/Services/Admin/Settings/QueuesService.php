            ...
            ->addColumn('aksi', function ($row) {
                ...
                if (auth()->user()->can('settings-queues.update')) {
                    $btnRetry = '<form action="' . route('settings.queues.retry', $row->id) . '" method="POST" class="inline">
                        ' . csrf_field() . '
                        <button type="submit" title="Retry Job" class="w-[30px] h-[30px] flex items-center justify-center border rounded-[5px] text-primary hover:bg-light-900 cursor-pointer">
                            <i class="ti ti-rotate-clockwise text-[16px]"></i>
                        </button>
                    </form>';
                }
                ...
                if (auth()->user()->can('settings-queues.delete')) {
                    $btnForget = '<button type="button" title="Hapus Failed Job"
                        ...
                        class="btn-forget-job w-[30px] h-[30px] flex items-center justify-center border rounded-[5px] text-danger hover:bg-light-900 cursor-pointer">
