        ...
        return DataTables::of($tasks)
            ...
            ->addColumn('status_badge', function ($row) {
                ...
                if (!empty($row['last_status'])) {
                    if ($row['last_status'] === 'SUCCESS') {
                        return '<span class="inline-flex items-center py-0.5 px-2 rounded-full text-xs font-semibold bg-success/10 text-success border border-success/20"><i class="ti ti-check mr-1"></i> Sukses ' . ($row['last_duration'] ? "({$row['last_duration']})" : '') . '</span>';
                    }
                    return '<span class="inline-flex items-center py-0.5 px-2 rounded-full text-xs font-semibold bg-danger/10 text-danger border border-danger/20"><i class="ti ti-alert-circle mr-1"></i> Gagal</span>';
                }
            ...
            ->addColumn('aksi', function ($row) {
                ...
                if (auth()->user()->can('settings-schedulers.update')) {
                    $btnRun = '<button type="button" title="Jalankan Task Sekarang" class="btn-run-task w-[30px] h-[30px] flex items-center justify-center border rounded-[5px] text-primary hover:bg-light-900 cursor-pointer"
                ...
                if ($row['can_edit'] && auth()->user()->can('settings-schedulers.update')) {
                    $btnEdit = '<button type="button" title="Edit Task" class="btn-edit-task w-[30px] h-[30px] flex items-center justify-center border rounded-[5px] text-warning hover:bg-light-900 cursor-pointer"
                        data-id="' . e($row['db_id']) . '">
                ...
                if ($row['can_edit'] && auth()->user()->can('settings-schedulers.delete')) {
                    $btnDelete = '<button type="button" title="Hapus Task" class="btn-delete-task w-[30px] h-[30px] flex items-center justify-center border rounded-[5px] text-danger hover:bg-light-900 cursor-pointer"
                        data-id="' . e($row['db_id']) . '"
                        data-name="' . e($row['name']) . '">
        ...
        return DataTables::of($logs)
            ...
            ->addColumn('status_badge', function ($row) {
                if ($row->status === SchedulerStatusEnum::SUCCESS) {
                    return '<span class="inline-flex items-center py-0.5 px-2 rounded-full text-xs font-semibold bg-success/10 text-success border border-success/20"><i class="ti ti-check mr-1"></i> Sukses</span>';
                }
                return '<span class="inline-flex items-center py-0.5 px-2 rounded-full text-xs font-semibold bg-danger/10 text-danger border border-danger/20"><i class="ti ti-alert-triangle mr-1"></i> Gagal</span>';
            })
            ...
            ->addColumn('aksi', function ($row) {
                return '<button type="button" class="btn-log-detail btn bg-light border border-borderColor text-gray-700 hover:bg-gray-200 py-1 px-2.5 rounded text-xs inline-flex items-center gap-1"
